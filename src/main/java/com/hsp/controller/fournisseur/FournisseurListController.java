...
public class FournisseurListController implements Initializable {
    ...
    @FXML private TableColumn<Fournisseur, String> idCol;
    @FXML private TableColumn<Fournisseur, String> nomCol;
    @FXML private TableColumn<Fournisseur, String> contactCol;
    @FXML private TableColumn<Fournisseur, String> emailCol;
    @FXML private TableColumn<Fournisseur, String> telephoneCol;
    @FXML private TableColumn<Fournisseur, String> adresseCol;
    @FXML private TableColumn<Fournisseur, String> dateCreationCol;
    ...
    private void configurerColonnes() {
        ...
        contactCol.setCellValueFactory(cellData ->
                new SimpleStringProperty(cellData.getValue().getContact() != null ? cellData.getValue().getContact() : ""));
        ...
        adresseCol.setCellValueFactory(cellData -> {
            ...
            if (adresse != null && adresse.length() > 40) adresse = adresse.substring(0, 37) + "...";
    ...
    private void rechercher() {
        ...
        if (texte.isEmpty()) { fournisseurs.addAll(tous); return; }

        for (Fournisseur fournisseur : tous) {
            boolean correspond = false;
            if (String.valueOf(fournisseur.getId_fournisseur()).contains(texte))                             correspond = true;
            if (fournisseur.getNom()       != null && fournisseur.getNom().toLowerCase().contains(texte))   correspond = true;
            if (fournisseur.getContact()   != null && fournisseur.getContact().toLowerCase().contains(texte)) correspond = true;
            if (fournisseur.getEmail()     != null && fournisseur.getEmail().toLowerCase().contains(texte)) correspond = true;
            if (fournisseur.getTelephone() != null && fournisseur.getTelephone().contains(texte))           correspond = true;
            if (correspond) fournisseurs.add(fournisseur);
        }
    }
    ...
    private void supprimerFournisseur() {
        ...
        if (resultat.isPresent() && resultat.get() == ButtonType.OK) {
            ...
            if (succes) {
                enregistrerHistorique("SUPPRESSION", "fournisseur", selection.getId_fournisseur(),   // ← AJOUT
                        "Suppression du fournisseur : " + selection.getNom());
                chargerFournisseurs();
    ...
    @FXML
    private void rafraichir() { recherche.clear(); chargerFournisseurs(); }
